 ...
 *
 * When invoked with libFuzzer flags (-runs=..., -max_total_time=...) the harness iterates like any
 * libFuzzer target; when invoked with a single file argument it replays that input once — so the
...
#endif
#ifndef SCRIPT_PATH
#define SCRIPT_PATH "/mayhem/mayhem/fuzz_parsers.py"
#endif
